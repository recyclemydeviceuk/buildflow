  ...
  return (
    <>
      <button
        ...
        className={`rounded-xl border border-[#D8E1F0] bg-white px-4 py-2.5 text-left transition-all ${
          disabled
      ...
      >
        <div className="flex items-center gap-3">
          {selected?.dotColor ? (
            <span className="w-2 h-2 rounded-full shrink-0" style={{ background: selected.dotColor }} />
          ) : null}
          <div className="min-w-0 flex-1">
            <p className="text-sm font-semibold text-[#334155] truncate">{selected?.label || placeholder}</p>
          </div>
          <ChevronDown
            size={16}
              ...
              <div className="overflow-y-auto p-2">
                {filteredOptions.length === 0 ? (
                  <div className="px-4 py-8 text-center">
                    <p className="text-sm font-semibold text-[#475569]">No matches found</p>
                    <p className="text-xs text-[#94A3B8] mt-1">Try a different search.</p>
                  </div>
                ) : (
                  filteredOptions.map((option) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        {option.dotColor ? (
                          <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ background: option.dotColor }} />
                        ) : (
                        ...
                        {option.value === value ? <Check size={16} className="text-[#1D4ED8] shrink-0" /> : null}
                      </div>
                    </button>
                  ))
                )}
